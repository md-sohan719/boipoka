...
@section('content')
    <div class="content container-fluid">
        <div class="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-3">
            ...
            <a class="btn btn-primary text-capitalize" href="{{ route('admin.products.product-gallery') }}">
                {{ translate('add_info_from_gallery') }}
            </a>
        </div>

        <form class="product-form text-start form-advance-validation form-advance-file-validation"
            action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" id="product_form"
            data-ajax="true" novalidate="novalidate">
            @csrf
        ...
        @if (getActiveAIProviderConfigCache())
            <div class="floating-ai-button">
                <button type="button" class="btn btn-lg rounded-circle shadow-lg" data-bs-toggle="modal"
                    ...
                    <span class="position-relative z-1 text-white d-flex flex-column gap-1 align-items-center">
                        <img width="16" height="17"
                            src="{{ dynamicAsset(path: 'public/assets//back-end/img/ai/hexa-ai.svg') }}" alt="">
    ...
    <span id="product-add-update-messages" data-are-you-sure="{{ translate('are_you_sure') }}"
        data-want-to-add="{{ translate('want_to_add_this_product') }} ?" data-yes-word="{{ translate('yes') }}"
        data-no-word="{{ translate('no') }}"></span>
...
@push('script')
    ...
    <script src="{{ dynamicAsset(path: 'public/assets/backend/admin/js/AI/products/product-title-autofill.js') }}">
    </script>
    <script src="{{ dynamicAsset(path: 'public/assets/backend/admin/js/AI/products/product-title-autofill.js') }}">
    </script>
    <script src="{{ dynamicAsset(path: 'public/assets/backend/admin/js/AI/products/product-description-autofill.js') }}">
    </script>
    ...
    <script src="{{ dynamicAsset(path: 'public/assets/backend/admin/js/AI/products/product-variation-setup.js') }}">
    </script>
